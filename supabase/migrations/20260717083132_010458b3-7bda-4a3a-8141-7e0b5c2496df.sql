...
DECLARE
  ...
  c record;
BEGIN
  SELECT id, store_id, customer_id, invoice_number, total_amount, address_token_expires_at,
  ...
  SELECT name, mobile INTO c FROM customers WHERE id = r.customer_id;
  ...
  RETURN jsonb_build_object(
    ...
    'shipping', jsonb_build_object(
      'name', COALESCE(NULLIF(r.shipping_name, ''), c.name),
      'phone', COALESCE(NULLIF(r.shipping_phone, ''), c.mobile),

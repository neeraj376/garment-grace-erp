...
DECLARE
  ...
  _cust record;
BEGIN
  SELECT id, store_id, customer_id, address_token_expires_at INTO r
  ...
  IF _customer_id IS NOT NULL THEN
    SELECT total_spent, visit_count INTO _cust FROM customers WHERE id = _customer_id;
    IF _cust IS NOT NULL THEN
      UPDATE customers
      SET total_spent = COALESCE(_cust.total_spent, 0) + (SELECT COALESCE(total_amount, 0) FROM invoices WHERE id = r.id),
          visit_count = COALESCE(_cust.visit_count, 0) + 1,
          address_line1 = btrim(p_line1),
          address_line2 = NULLIF(btrim(p_line2),''),
          city = btrim(p_city),
          state = btrim(p_state),
